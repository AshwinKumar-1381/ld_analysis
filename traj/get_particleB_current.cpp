/*
 ...
 particle currents of particle B along y - direction
...
int main(int argc, char *argv[])
{
    ...
    while(!feof(file_traj))
    {
        ...
        if(frame_nr == int((t_ss_start + N_avg)/frame_w) and frame_nr < int(t_ss_end/frame_w))      
        {
            ...
            {
                fgets(pipeString, 500, file_traj);
                sscanf(pipeString, "%c %f %f %*f", &id[i], &rx[i], &ry_t[i]);   
            }
            ...
            for(i = 0; i < n_bin; i++)
            {
                ...
                // To remove bins with very less no.of particles
                if(bin_count[i] <= 10 and i <= int(n_bin/2))
                {
                    vel[i+1] += vel[i];
                ...
                else if(bin_count[i] <= 10 and i >= int(n_bin/2))
                {
                    vel[i-1] += vel[i];
                    bin_count[i-1] += bin_count[i];
                    vel[i] = 0.0;
                ...
                //printf("%d\n", bin_count[i]);
                if(bin_count[i] != 0) vel[i] /= bin_count[i];
